...
export const getNearbyFacilities = async (address: string): Promise<MapPlace[]> => {
  try {
    ...
    const categories = [
      ...
      { type: 'subway_station', label: '駅', rankby: 'distance', count: 3, maxDistance: 5000 },
    ...
    for (const cat of categories) {
      ...
      if (nearbyData.results && nearbyData.results.length > 0) {
        // 駅の場合は名前に「駅」が含まれており、かつバス停などを除外する
        const filteredResults = cat.label === '駅' 
          ? nearbyData.results.filter((p: any) => 
              p.name.includes('駅') && 
              !p.name.includes('バス') && 
              !p.name.includes('停留所') &&
              !p.name.includes('行）') && // バス路線の行き先表示対策
              !p.name.includes('方面')
            )
          : nearbyData.results;
    ...
    if (allPlaces.length > 0) {
      ...
      if (distData.rows && distData.rows[0].elements) {
        ...
        distData.rows[0].elements.forEach((el: any, idx: number) => {
          if (el.status === 'OK') {
            ...
            if (distanceValue <= maxDist) {
              ...
              allPlaces[idx].distanceValue = distanceValue;
        ...
        // 全体を距離の近い順にソート
        const sortedPlaces = finalPlaces.sort((a, b) => (a.distanceValue || 0) - (b.distanceValue || 0));
        
        // 重複排除（名前で判断）
        const uniquePlaces: MapPlace[] = [];
