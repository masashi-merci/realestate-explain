...
4. **周辺施設と距離**: ユーザーから提供される「周辺施設データ」を基に、主要な施設を文章の中に自然な形で組み込んでください。データは距離の近い順に並んでいます。施設名と道のり距離（m）は正確に記載してください。バス停を「駅」と誤認して記載しないでください。
5. **最寄り駅の特定**: 提供されたデータの中で距離が近い順に最大3つの「駅」を特定し、それぞれの駅名と徒歩分数を明記してください。最も近い駅をメインの最寄り駅として扱ってください。
6. **物件種別による制限**: 
...
[FACILITIES_JSON]
[
  {"name": "施設名", "distance": "〇〇m", "category": "駅|学校|コンビニ|スーパー|郵便局|病院|その他"}
]
[/FACILITIES_JSON]
※「駅」カテゴリーには、特定した最大3つの最寄り駅を必ず含めてください。
※「アクセス」や「主要駅へのアクセス」といったカテゴリーは絶対に使用しないでください。
---
`;
