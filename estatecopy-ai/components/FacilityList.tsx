...
const FacilityList: React.FC<FacilityListProps> = ({ facilities, isLoading }) => {
  ...
  const categories = ['駅', 'コンビニ', '飲食', '学校', '郵便局', 'その他'] as const;
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mt-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-lg font-bold text-gray-900">周辺施設</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {categories.map(category => {
          ...
          return (
            <div key={category} className="group rounded-xl p-4 border transition-all duration-300 bg-white border-gray-100 hover:border-orange-200 hover:shadow-md">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[10px] text-gray-300 font-bold">{categoryFacilities.length} items</span>
              </div>
              <ul className="space-y-2">
                {categoryFacilities.map((f, i) => (
                  <li key={i} className="flex justify-between items-start group/item">
                    <div className="flex items-center overflow-hidden">
                      <span className="text-sm font-medium leading-tight transition-colors truncate text-gray-700 group-hover/item:text-black">
                        {f.name}
                      </span>
                      {f.url && (
                        <a 
                    ...
                    {f.distance && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded whitespace-nowrap font-mono text-gray-400 bg-gray-50">
                        {f.distance}
                      </span>
                    )}
                  </li>
